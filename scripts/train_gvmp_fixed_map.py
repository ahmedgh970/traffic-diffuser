...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, idx):      
        ...
        null_file = 'sd_nuscenes_v1.0-trainval_scene-0652.npy'
        map_null = np.load(os.path.join(self.map_path, null_file))
        map_null = torch.tensor(map_null, dtype=torch.float32)
        return data_tensor, map_tensor, map_null
...
def main(config):
    ...
    for epoch in range(num_epochs):
        ...
        for data, mp, mp_null in loader:
            x = data[:, :, hist_length:, :].to(device)
            h = data[:, :, :hist_length, :].to(device)
            mp = mp.to(device)
            mp_null = mp_null.to(device)
            model_kwargs = dict(h=h, mp=mp, mp_null=mp_null)
