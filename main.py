...
def run_sample(ckpt, gpu_id, module="model_td", model="TrafficDiffuser-S", nagents=46, histl=8, seql=5, n_steps=250, 
    ...
    # Base command
    command = f"python sample.py --cuda-device {gpu_id} --ckpt {ckpt} \
...
if __name__ == '__main__':
    ...
    process_1 = multiprocessing.Process(
        target=run_sample,
        args=("/data/ahmed.ghorbel/workdir/autod/traffic-diffuser/results/001-TrafficDiffuser-S/checkpoints/0084000.pt", 0, "model_td")
    )
    #process_2 = multiprocessing.Process(
    #    target=run_sample,
    #    args=("/data/ahmed.ghorbel/workdir/autod/traffic-diffuser/results/001-TrafficDiffuser-S/checkpoints/0084000.pt", 1, "model_td")
    #)
